...
_Généré le 2026-04-17 — prompt-50_
...
# Copier/coller le contenu de chaque migration dans l'ordre numérique
```
...
|---|---------|-------------|----------|
| 1 | `20260415000019_plan_configs.sql` | Table `plan_configs` (durées/prix par plan) + données initiales | 🔴 CRITIQUE — PricingSection et AdminLicences cassés sans elle |
| 2 | `20260415000020_admin_account_actions.sql` | `documents.client_id` nullable + colonne `users.statut` | 🔴 CRITIQUE — fiche compte Admin crash sans `users.statut` |
| 3 | `20260415000021_preview_anon_users.sql` | RLS anon sur `users` pour page preview publique | 🟡 IMPORTANT — `cmName` non résolu sur PreviewPage |
...
| 5 | `20260416000023_elite_requests.sql` | Table `elite_requests` (demandes plan Elite) + RLS | 🟡 IMPORTANT — EliteContactModal crash sans la table |
| 6 | `20260416000024_launch_settings.sql` | INSERT `launch_date` + `show_countdown` dans `site_settings` | 🟢 NORMAL — countdown landing non affiché |
| 7 | `20260416000025_update_plan_names.sql` | UPDATE `plans.nom` anciens noms → Découverte / CM Pro / Studio / Elite | 🟢 NORMAL — noms plans affichés depuis BDD incorrects |

---

## SQL complet par migration

### 1. plan_configs — 🔴 CRITIQUE
...
### 2. admin_account_actions — 🔴 CRITIQUE
...
### 3. preview_anon_users — 🟡 IMPORTANT

Voir fichier complet : `supabase/migrations/20260415000021_preview_anon_users.sql`

Résumé : ajoute des policies RLS permettant aux utilisateurs anonymes de lire les données `users` nécessaires à la page preview publique (résolution du nom du CM).

---

### 4. documents_remise — 🟡 IMPORTANT
...
### 7. update_plan_names — 🟢 NORMAL

```sql
UPDATE public.plans SET nom = 'Découverte' WHERE slug = 'freemium';
UPDATE public.plans SET nom = 'CM Pro'     WHERE slug = 'solo_standard';
...
FROM public.plan_configs ORDER BY plan_type, duree_mois;

-- Vérifier colonnes documents
SELECT column_name FROM information_schema.columns
WHERE table_name = 'documents'
  AND column_name IN ('client_id', 'remise_pct', 'remise_montant');

-- Vérifier elite_requests
SELECT COUNT(*) FROM public.elite_requests;

-- Vérifier site_settings
SELECT key, value FROM public.site_settings
WHERE key IN ('launch_date', 'show_countdown');
```
